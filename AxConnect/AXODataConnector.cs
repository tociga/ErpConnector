...
namespace AxConnect
{
    public class AXODataConnector
    {
        ...
        {
            var rand = new Random();
            string productNumber = "AGRPOC2";
            string productName = "AGR Prof of Concept2";
            string productSearchName = "AGRProfOfConcept2";
            ...
            //rdp.ProductNumber = productNumber;
            ...
            master.ProductDimensionGroupName = "ColorSize";
            master.ProductName = productName;
            master.ProductNumber = productNumber;
            master.ProductSearchName = productSearchName;
            master.ProductSizeGroupId = "DRESSES"; // i * 2 where i >= 0 && 6<=i
            ...
            var r = AXServiceConnector.CallOdataEndpointPost("ProductMasters", null, master).Result;
        ...
        {
            AGROrderWriteDTO order = new AGROrderWriteDTO();
            order.ARGId = "4";
            order.OrderFrom = "1004"; //vendor number;
            order.OrderTo = "DC"; //warehouse;
            order.OrderType = AGROrderType.PO;
            order.ReceiveDate = DateTime.Now.Date.AddDays(20);
            order.OrderStatus = AGROrderStatus.Created;

            var r = AXServiceConnector.CallOdataEndpointPost<AGROrderWriteDTO>("AGROrders", null, order);

            AGROrderLineWriteDTO line = new AGROrderLineWriteDTO();
            ...
            var l = AXServiceConnector.CallOdataEndpointPost<AGROrderLineWriteDTO>("AGROrderLines", null, line);
            order.OrderStatus = AGROrderStatus.Ready;

            r = AXServiceConnector.CallOdataEndpointPut<AGROrderWriteDTO>("AGROrders", null, order);
        }


        public void CreateToTest()
        {
            AGROrderWriteDTO order = new AGROrderWriteDTO();
            order.ARGId = "3";
            order.OrderFrom = "DC"; //vendor number;
            order.OrderTo = "ANNAPOL"; //warehouse;
            order.OrderType = AGROrderType.TO;
            order.ReceiveDate = DateTime.Now.Date.AddDays(20);
            order.OrderStatus = AGROrderStatus.Created;
